...
- **Host service exposure via port forwarding:** when `--expose-port` is used, `host.docker.internal` is added as a route into the container, making all host ports reachable — not just the ones forwarded by socat. Socat sets up convenient `localhost` listeners for the specified ports, but code in the container can connect to `host.docker.internal:<any-port>` directly. This is opt-in and the risk is that a compromised agent could probe or interact with any host service that binds to localhost (e.g., databases, dev servers). Note that `--expose-port` only adds a *hostname* alias — the container's default gateway is the host's bridge IP regardless, so any host service bound to `0.0.0.0` (or the bridge) is reachable from the container even without `--expose-port`.

- **Host root via a reachable Docker daemon:** if the host runs its Docker daemon in the default rootful mode (daemon runs as root) *and* exposes it over an unauthenticated TCP socket reachable from the container (e.g. bound to `0.0.0.0`/the bridge, or Docker Desktop's "expose daemon without TLS" toggle), a compromised agent can call the Docker API via the host bridge and launch a *sibling* root container that bind-mounts `/` writable — full host root, no container escape required. The blast radius is critical but the prerequisites are narrow (an unauthenticated rootful daemon is itself a misconfiguration). Mitigations: run a **rootless** Docker daemon (a container's "root" then maps to your unprivileged UID, so the escalation fails), and never expose the daemon over unauthenticated TCP. coding-capsule prints a warning when it detects a rootful daemon.
